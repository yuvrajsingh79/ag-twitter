...
func AuthRedirectHandler(config *Config, failure http.Handler) http.Handler {
	if failure == nil {
		failure = DefaultFailureHandler
	}
	fn := func(w http.ResponseWriter, req *http.Request) {
...
func CookieTempHandler(config CookieConfig, success, failure http.Handler) http.Handler {
	if failure == nil {
		failure = DefaultFailureHandler
	}
	fn := func(w http.ResponseWriter, req *http.Request) {
		...
		if err == nil {
			...
			http.SetCookie(w, NewCookie(config, requestSecret))
			success.ServeHTTP(w, req.WithContext(ctx))
			return
		}
...
func CallbackHandler(config *Config, success, failure http.Handler) http.Handler {
	if failure == nil {
		failure = DefaultFailureHandler
	}
	fn := func(w http.ResponseWriter, req *http.Request) {
