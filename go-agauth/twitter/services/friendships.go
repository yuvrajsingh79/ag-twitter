...
func (s *FriendshipService) Create(params *FriendshipCreateParams) (*users.User, *http.Response, error) {
	user := new(users.User)
...
func (s *FriendshipService) Destroy(params *FriendshipDestroyParams) (*users.User, *http.Response, error) {
	user := new(users.User)
	apiError := new(users.APIError)
