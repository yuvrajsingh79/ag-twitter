...
func TestStatusService_Show(t *testing.T) {
	...
	params := &users.StatusShowParams{ID: 5441, IncludeEntities: Bool(false)}
	tweet, _, err := client.Statuses.Show(589488862814076930, params)
	expected := &users.Tweet{User: &users.User{ScreenName: "dghubble"}, Text: ".@audreyr use a DONTREADME file if you really want people to read it :P"}
	assert.Nil(t, err)
...
func TestStatusService_Lookup(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusLookupParams{ID: []int64{20}, TrimUser: Bool(true)}
	tweets, _, err := client.Statuses.Lookup([]int64{573893817000140800}, params)
	expected := []users.Tweet{users.Tweet{ID: 20, Text: "just setting up my twttr"}, users.Tweet{ID: 573893817000140800, Text: "Don't get lost #PaxEast2015"}}
	assert.Nil(t, err)
...
func TestStatusService_Update(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusUpdateParams{MediaIds: []int64{123456789, 987654321}, Lat: Float(37.826706), Long: Float(-122.422190)}
	tweet, _, err := client.Statuses.Update("very informative tweet", params)
	expected := &users.Tweet{ID: 581980947630845953, Text: "very informative tweet"}
	assert.Nil(t, err)
	assert.Equal(t, expected, tweet)
}
...
func TestStatusService_Retweet(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusRetweetParams{TrimUser: Bool(true)}
	tweet, _, err := client.Statuses.Retweet(20, params)
	expected := &users.Tweet{ID: 581980947630202020, Text: "RT @jack: just setting up my twttr", RetweetedStatus: &users.Tweet{ID: 20, Text: "just setting up my twttr"}}
	assert.Nil(t, err)
	assert.Equal(t, expected, tweet)
}
...
func TestStatusService_Unretweet(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusUnretweetParams{TrimUser: Bool(true)}
...
func TestStatusService_Retweets(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusRetweetsParams{Count: 2}
	retweets, _, err := client.Statuses.Retweets(20, params)
	expected := []users.Tweet{users.Tweet{Text: "RT @jack: just setting up my twttr"}, users.Tweet{Text: "RT @jack: just setting up my twttr"}}
	assert.Nil(t, err)
...
func TestStatusService_Destroy(t *testing.T) {
	...
	client := NewClient(httpClient)
	params := &users.StatusDestroyParams{TrimUser: Bool(true)}
	tweet, _, err := client.Statuses.Destroy(40, params)
	// feed Biz Stone a sammich, he deletes sammich Tweet
	expected := &users.Tweet{ID: 40, Text: "wishing I had another sammich"}
	assert.Nil(t, err)
	assert.Equal(t, expected, tweet)
}
