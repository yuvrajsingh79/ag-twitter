...
func TestFavoriteService_List(t *testing.T) {
	...
	expected := []users.Tweet{users.Tweet{Text: "Gophercon talks!"}, users.Tweet{Text: "Why gophers are so adorable"}}
	assert.Nil(t, err)
...
func TestFavoriteService_Destroy(t *testing.T) {
	...
	assert.Nil(t, err)
	expected := &users.Tweet{ID: 581980947630845953, Text: "very unhappy tweet"}
	assert.Equal(t, expected, tweet)
}
